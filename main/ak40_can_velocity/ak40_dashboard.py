...
PORT="COM14"; BAUD=115200; MAXPTS=500
class Dash(QMainWindow):
    ...
    def connect(self):
        try:
            ...
            self.status.setText("Connected "+PORT)
        except Exception as ex:
            self.ser=None
            self.status.setText("Connect failed: "+str(ex))
    def send(self):
        if not self.ser or not self.ser.is_open:
            self.status.setText("Not connected — close Serial Monitor / idf monitor first")
            return
        cmd=self.box.text().strip()
        if not cmd:
            return
        self.ser.write((cmd+"\n").encode("utf-8"))
        self.ser.flush()
        ...
        if self.ser and self.ser.is_open:
            while self.ser.in_waiting:
                ...
                try:
                    ...
                    vals = {
                        "target": fnum(d.get("target_erpm"), 0),
                        "cmd": fnum(d.get("current_erpm"), 0),
                        "meas": fnum(d.get("measured_erpm"), 0),
                        "pos": fnum(d.get("pos"), 0),
                        "temp": fnum(d.get("tempC", d.get("temp")), 0),
                        "cur": fnum(d.get("current"), 0),
                        "torque": fnum(d.get("torque_est", d.get("torque")), 0),
                    }
                except Exception:
                    continue
                self.d["t"].append(t)
                ...
                self.lab["state"].setText(d.get("state", ""))
                self.lab["target"].setText(str(vals["target"]))
                self.lab["current"].setText(str(vals["cmd"]))
                self.lab["measured"].setText(str(vals["meas"]))
                self.lab["position"].setText(str(vals["pos"]))
                self.lab["currentA"].setText(str(vals["cur"]))
                self.lab["torque"].setText(str(vals["torque"]))
                self.lab["temp"].setText(str(vals["temp"]))
                self.lab["err"].setText(d.get("err", "0"))
        if self.d["t"]:
            x = list(self.d["t"])
            self.l1.setData(x, list(self.d["target"]))
            self.l2.setData(x, list(self.d["cmd"]))
            self.l3.setData(x, list(self.d["meas"]))
            self.l4.setData(x, list(self.d["pos"]))
            self.l5.setData(x, list(self.d["cur"]))
            self.l6.setData(x, list(self.d["torque"]))
            self.l7.setData(x, list(self.d["temp"]))
